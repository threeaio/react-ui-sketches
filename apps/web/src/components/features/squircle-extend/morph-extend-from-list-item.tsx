...
export function MorphExtendFromListItem({ title, icon, description, isExpanded, setIsExpanded, close }: MorphExtendFromListItemProps) {


	return (
        <MySquircle layout className={cn('dark:bg-gray-900 bg-gray-100', isExpanded ? "w-3xl absolute top-[calc(var(--height-header)+1rem)] left-1/2 -translate-x-1/2" : "")} transition={{ type: "spring", bounce: 0.25 }}>
        <div className={cn(" p-6 transition-all duration-300", isExpanded ? "pt-12 px-12 pb-12" : "p-6")}>
                <AnimatePresence>
                {
                    isExpanded && (
                        <motion.div 
                            ...
                            className="flex items-center justify-between ">
                                ...
                                <Button className="cursor-pointer" variant="ghost" size="icon" onClick={() => close()}>
